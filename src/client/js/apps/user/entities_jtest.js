...
describe('user/entities', function() {
  ...
  it('requires valid email addresses', function() {
    var user = new this.Entities.User({ email: this.data.invalid_email });
  ...
  it('does not require password to be defined', function() {
    var user = new this.Entities.User({ email: this.data.valid_email });
  ...
  it('requires password to be alphanumeric if defined', function() {
    var user = new this.Entities.User({ email: this.data.valid_email, password: this.data.invalid_char_password });
    var validationErrors = user.validate(user.attributes);
    expect(validationErrors['password']).toBeDefined();
  });

  it('requires password to be at least 8 characters long', function() {
    var user = new this.Entities.User({ email: this.data.valid_email, password: this.data.invalid_short_password });
    var validationErrors = user.validate(user.attributes);
    expect(validationErrors['password']).toBeDefined();
  });

  it('validates with valid password and valid email', function() {
    var user = new this.Entities.User({ email: this.data.valid_email, password: this.data.valid_password });
    var validationErrors = user.validate(user.attributes);
