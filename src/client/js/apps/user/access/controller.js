          ...
          if(_.isEmpty(validation_errors)) {
            ...
            $.post('/api/user/access/local', submitted_data, function(resp_data, textStatus, jqXhr) {
              if(resp_data.status === 'success') {
                logger.debug('UserApp.Access - local_access post response -- access succeess, redirecting to profile');
                PF.trigger('user:profile');
              }
              else if(resp_data.status === 'failure') { // duplicate email, server side validation error or ?
                logger.debug('UserApp.Access - local_access post response -- access failure: ' + resp_data.message);
                view.model.set({ message: resp_data.message });
              }
              else {
                logger.error('UserApp.Access - local_access post response -- unknown status: ' + resp_data.status +
